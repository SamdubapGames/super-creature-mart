...
(function testFindPartIndex() {
  ...
  messages.push(testLine(p1,
    ...
    '0', t1,
    'for문이 i=0부터 시작하는지 확인하세요. parts[i] === part 일 때 i를 return해야 합니다.'));
...
(function testGenerateRoute() {
  ...
  messages.push(testLine(p1,
    '🎲 반환값이 배열인지',
    '배열', t1,
    'let result = [] 로 빈 배열을 만들고 push한 뒤 return result 해야 합니다.'));
  ...
  messages.push(testLine(p2,
    '🎲 length=5 요청 → 배열 길이 5',
    '길이: 5', Array.isArray(t2) ? "길이: " + t2.length : t2,
    'for (let i = 0; i < length; i++) 에서 length가 맞는지 확인하세요.'));
  ...
  messages.push(testLine(p3,
    '🎲 length=1 → 길이 1',
    '길이: 1', Array.isArray(t3) ? "길이: " + t3.length : t3,
    'for문이 length번 정확히 도는지 확인하세요.'));
  ...
  messages.push(testLine(p4,
    '🎲 length=0 → 빈 배열 (경계값)',
    '[]', t4,
    'length가 0이면 for문이 한 번도 안 돌아서 빈 배열이 반환됩니다. 별도 처리 없이 그냥 return result 하면 됩니다.'));
  ...
  messages.push(testLine(p5,
    ...
    '전부 유효', allValid ? '전부 유효' : '유효하지 않은 값: ' + invalidItem,
  ...
  messages.push(testLine(p6,
    ...
    '2종류 이상', kinds + "종류: " + Object.keys(seen).join(", "),
  ...
  messages.push(testLine(p7,
    '🛡️ 원본 PARTS 배열이 바뀌지 않는지 (부작용 검사)',
    '원본 유지', p7 ? '원본 유지' : '원본 변경됨!',
    'parts.splice() 를 쓰지 말고, result.push(parts[idx]) 만 해야 합니다. 같은 부위가 반복 뽑혀도 됩니다.'));
...
(function testCheckMatch() {
  ...
  var t1 = checkMatch("머리", "검", PAIRS);
  var p1 = t1 === true;
  if (!p1) passed = false;
  messages.push(testLine(p1,
    '🎯 checkMatch("머리", "검", PAIRS) — 올바른 짝 → true',
    'true', t1,
    'if (pairs[i].part === part && pairs[i].item === item) 일 때 return true 해야 합니다.'));

  var t2 = checkMatch("꼬리", "떡", PAIRS);
  var p2 = t2 === true;
  if (!p2) passed = false;
  messages.push(testLine(p2,
    '🎯 checkMatch("꼬리", "떡", PAIRS) → true',
    'true', t2,
    'pairs 배열을 for문으로 전체 순회하고 있는지 확인하세요.'));

  var t3 = checkMatch("머리", "방패", PAIRS);
  var p3 = t3 === false;
  if (!p3) passed = false;
  messages.push(testLine(p3,
    '🎯 checkMatch("머리", "방패", PAIRS) — 부위는 있지만 아이템 틀림 → false',
    'false', t3,
    '부위(part)만 확인하지 말고 아이템(item)도 함께 === 로 비교해야 합니다. && 조건을 확인하세요.'));

  var t4 = checkMatch("날개", "검", PAIRS);
  var p4 = t4 === false;
  if (!p4) passed = false;
  messages.push(testLine(p4,
    '🎯 checkMatch("날개", "검", PAIRS) — 없는 부위 → false',
    'false', t4,
    'for문 바깥(함수 맨 끝)에 return false; 가 있어야 합니다. 못 찾으면 false를 반환해야 합니다.'));

  var t5 = checkMatch("몸통", "방패", PAIRS);
  ...
  var origLen = PAIRS.length;
  checkMatch("머리", "검", PAIRS);
  var p6 = PAIRS.length === origLen;
  if (!p6) passed = false;
  messages.push(testLine(p6,
    '🛡️ 원본 PAIRS 배열이 바뀌지 않는지 (부작용 검사)',
    '길이 ' + origLen + ' 유지', 'length: ' + PAIRS.length,
    'PAIRS는 읽기만 해야 합니다.'));
...
(function testPlayTurn() {
  ...
  messages.push(testLine(p3,
    ...
    '"message" 포함', JSON.stringify(t2),
    '반환 객체의 키 이름이 정확히 message 인지 확인하세요.'));

  var inv4 = freshInv();
  var t4 = playTurn("머리", "검", inv4, PAIRS);
  var p4 = t4 && t4.success === true;
  if (!p4) passed = false;
  messages.push(testLine(p4,
    '⚔️ 올바른 짝("머리" + "검") → success: true',
    'success: true', t4 ? JSON.stringify(t4) : t4,
    'checkMatch(part, item, pairs) 가 true를 반환할 때 { success: true, ... } 를 반환해야 합니다.'));

  var p5 = !safeIncludes(inv4, "검");
  if (!p5) passed = false;
  messages.push(testLine(p5,
    '⚔️ 성공 후 "검"이 인벤토리에서 사라지는지',
    '"검" 제거됨', JSON.stringify(inv4),
    '성공 분기 안에서 useItem(inventory, item) 을 호출해야 합니다.'));

  var inv6 = freshInv();
  var t6 = playTurn("머리", "떡", inv6, PAIRS);
  var p6 = t6 && t6.success === false;
  if (!p6) passed = false;
  messages.push(testLine(p6,
    '⚔️ 틀린 짝("머리" + "떡") → success: false',
    'success: false', t6 ? JSON.stringify(t6) : t6,
