...
(function testFormatTipLine() {
    ...
    messages.push(
        testLine(
            ...
            '"입 → 떡"',
            t1,
            'return part + " → " + item; 형태인지 확인하세요. 화살표(→) 양쪽에 공백이 있어야 합니다.',
        ),
    );
    ...
    messages.push(
        testLine(
            p4,
            "✍️ part가 앞, item이 뒤에 오는지",
            '"눈"이 "활"보다 앞',
            t4,
            "함수 선언의 매개변수 순서를 확인하세요. 첫 번째가 part, 두 번째가 item입니다.",
        ),
    );
    ...
    messages.push(
        testLine(
            ...
            '"뾰족한발톱 → 독버섯파이"',
            t5,
            "단어를 직접 쓰지 말고 변수 part, item을 그대로 이어붙여야 합니다.",
        ),
    );
...
(function testShowNotebook() {
    ...
    var t1 = showNotebook(parts3, items3);
    ...
    messages.push(
        testLine(
            ...
            "세 줄 모두 포함",
            t1,
            "for문이 parts.length번 돌고 있는지 확인하세요. formatTipLine(parts[i], items[i]) 결과를 result에 이어붙이고 있나요?",
        ),
    );
        ...
        testLine(
            ...
            typeof t1 === "string"
                ? t1.indexOf("\n") !== -1
                    ...
                    : "\\n 없음"
                : t1,
            '각 팁 줄 사이에 "\\n"을 넣어야 합니다. 배열에 push 하고 마지막에 join("\\n") 써도 됩니다.',
        ),
    );
    ...
    messages.push(
        testLine(
            ...
            "입→떡 먼저, 꼬리→방패 마지막",
            t1,
            "for문을 i=0부터 parts.length 미만까지 순서대로 돌아야 합니다.",
        ),
    );

    var t5 = showNotebook(["배"], ["사과"]);
    ...
    messages.push(
        testLine(
            ...
            '"배 → 사과" 포함',
            t5,
            "for문이 배열 길이에 맞게 돌고 있는지 확인하세요. parts.length가 1이면 1번만 돌아야 합니다.",
        ),
    );

    var expected = formatTipLine("X", "Y");
    var t6 = showNotebook(["X"], ["Y"]);
    ...
    messages.push(
        testLine(
            ...
            '"' + expected + '" 포함',
            t6,
            "showNotebook 안에서 formatTipLine(parts[i], items[i]) 을 호출해서 그 결과를 이어붙이세요.",
        ),
    );

    var origParts = ["입", "머리"];
    var origItems = ["떡", "검"];
    var origPartsLen = origParts.length;
    var origItemsLen = origItems.length;
    showNotebook(origParts, origItems);
    var p7 =
        origParts.length === origPartsLen && origItems.length === origItemsLen;
    if (!p7) passed = false;
    messages.push(
        testLine(
            p7,
            ...
            "parts 길이: " +
                ...
                ", items 길이: " +
                origItems.length,
            "parts, items 배열은 읽기만 해야 합니다. push() 나 splice() 를 쓰지 않았는지 확인하세요.",
        ),
    );
...
console.log(
    "=== notebook.js 테스트 완료! 브라우저 화면에서 결과를 확인하세요 ===",
);
